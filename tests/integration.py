...
def test_end_to_end_with_caching(db_session):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert os.path.exists(f"{cache_dir}/invocations.parquet"), (
            "Invocations cache file not found"
        )
        assert os.path.exists(f"{cache_dir}/embeddings.parquet"), (
            "Embeddings cache file not found"
        )
        assert os.path.exists(f"{cache_dir}/clusters.parquet"), (
            "Clusters cache file not found"
        )
        assert os.path.exists(f"{cache_dir}/pd.parquet"), (
            "Persistence diagram cache file not found"
        )
        ...
        assert runs_df_before.shape == runs_df_cached.shape, (
            "Runs dataframe shapes don't match"
        )
        assert set(runs_df_before.columns) == set(runs_df_cached.columns), (
            "Runs dataframe columns don't match"
        )
        ...
        before_sorted = runs_df_before.sort("run_id").select([
            "run_id",
            "experiment_id",
            "initial_prompt",
            "seed",
        ])
        cached_sorted = runs_df_cached.sort("run_id").select([
            "run_id",
            "experiment_id",
            "initial_prompt",
            "seed",
        ...
        pl.testing.assert_frame_equal(
            before_sorted, cached_sorted, check_column_order=False
        )
        ...
        assert runs_df_cached["experiment_id"].is_not_null().all(), (
            "Found null experiment_id"
        )
        assert runs_df_cached["network"].is_not_null().all(), "Found null network"
        assert runs_df_cached["entropy"].is_not_null().all(), (
            "Found null entropy values"
        )
        ...
        assert invocations_df_cached["sequence_number"].min() == 0, (
            "Sequence numbers should start at 0"
        )
        assert invocations_df_cached["sequence_number"].max() == 49, (
            "Sequence numbers should end at 49"
        )

        # Check embeddings dataframe
        # With downsample=2, we should have ~25 embeddings per model
        assert embeddings_df_cached.height == 50, (
        ...
        assert 20 <= clusters_df_cached.height <= 30, (
            f"Expected ~25 cluster assignments, got {clusters_df_cached.height}"
        )
        ...
        for label in unique_labels:
            assert label in unique_texts or label == "OUTLIER", (
        ...
        assert pd_df_cached["homology_dimension"].min() >= 0, (
            "Negative homology dimension found"
        )
        ...
        if infinite_pd.height > 0:
            assert infinite_pd["death"].is_infinite().all(), (
                "Infinite persistence without infinite death"
            )
        ...
        assert invocation_run_ids.issubset(runs_run_ids), (
            "Found invocations with invalid run_ids"
        )

        # Every embedding should belong to an invocation
        embedding_inv_ids = set(
            embeddings_df_cached["invocation_id"].unique().to_list()
        )
        invocation_ids = set(invocations_df_cached["id"].unique().to_list())
        assert embedding_inv_ids.issubset(invocation_ids), (
            "Found embeddings with invalid invocation_ids"
        )
        ...
        assert cluster_emb_ids.issubset(embedding_ids), (
            "Found cluster assignments for non-existent embeddings"
        )

        # Every persistence diagram should belong to a run
        pd_run_ids = set(pd_df_cached["run_id"].unique().to_list())
        assert pd_run_ids.issubset(runs_run_ids), (
            "Found persistence diagrams with invalid run_ids"
        )
