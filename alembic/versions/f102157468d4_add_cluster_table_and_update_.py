...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.UniqueConstraint('clustering_result_id', 'cluster_id', name='unique_cluster_per_result')
    )
    
    # Create indexes
    op.create_index('ix_cluster_clustering_result_id', 'cluster', ['clustering_result_id'])
    op.create_index('ix_cluster_medoid_embedding_id', 'cluster', ['medoid_embedding_id'])
    ...
    op.create_table(
        ...
        sa.UniqueConstraint('embedding_id', 'clustering_result_id', name='unique_embedding_clustering')
    )
    
    # Create indexes on new table
    op.create_index('ix_embeddingcluster_new_embedding_id', 'embeddingcluster_new', ['embedding_id'])
    op.create_index('ix_embeddingcluster_new_clustering_result_id', 'embeddingcluster_new', ['clustering_result_id'])
    op.create_index('ix_embeddingcluster_new_cluster_id', 'embeddingcluster_new', ['cluster_id'])
