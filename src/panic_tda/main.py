        ...
        for experiment in experiments:
            ...
            if verbose:
                ...
                typer.echo(
                    ...
                    f"elapsed: {naturaldelta(elapsed)}"
                )
...
def script(
    ...
    with get_session_from_connection_string(db_str) as session:
        from panic_tda.schemas import Invocation, InvocationType, Embedding
        from sqlmodel import select
        import time
        from datetime import datetime, timedelta
        
        # Query for all IMAGE invocations
        logger.info("Querying for IMAGE invocations...")
        
        # Get total count first
        total_count = session.exec(
            select(func.count()).select_from(Invocation).where(
                Invocation.type == InvocationType.IMAGE
            )
        ).one()
        
        logger.info(f"Found {total_count} IMAGE invocations in the database")
        
        if total_count == 0:
            logger.info("No IMAGE invocations found. Exiting.")
            return
        
        # Define the new image embedding models
        image_embedding_models = ["NomicVision", "JinaClipVision"]
        
        # Check how many embeddings already exist for each model
        for model in image_embedding_models:
            existing_count = session.exec(
                select(func.count()).select_from(Embedding).join(Invocation).where(
                    Invocation.type == InvocationType.IMAGE,
            ...
            logger.info(
                f"{model}: {existing_count}/{total_count} embeddings exist "
                f"({missing_count} to be created)"
            )
        ...
        for batch_num, offset in enumerate(range(0, total_count, batch_size), 1):
            ...
            # Get batch of image invocations
            invocations = session.exec(
                select(Invocation)
                .where(Invocation.type == InvocationType.IMAGE)
                .offset(offset)
                .limit(batch_size)
            ).all()
            
            # Extract invocation IDs
            invocation_ids = [str(inv.id) for inv in invocations]
            
            logger.info(
                f"Processing batch {batch_num}/{total_batches}: "
                f"{len(invocation_ids)} invocations (offset={offset})"
            )
            
            # Check which invocations already have embeddings for each model
            for embedding_model in image_embedding_models:
                # Count existing embeddings for this batch and model
                existing_embeddings = session.exec(
                    select(func.count()).select_from(Embedding).where(
                        Embedding.invocation_id.in_([inv.id for inv in invocations]),
                        Embedding.embedding_model == embedding_model,
                        Embedding.vector.is_not(None)
                    )
                ).one()
                
                need_embeddings = len(invocations) - existing_embeddings
                
                if need_embeddings == 0:
                    logger.info(
                        f"Skipping {embedding_model} for batch {batch_num} - "
                        f"all {len(invocations)} embeddings already exist"
                    )
                    continue
                
                logger.info(
                    f"Creating {need_embeddings} new embeddings with {embedding_model} "
                    f"({existing_embeddings} already exist in this batch)"
                )
            
            # Import what we need for embedding creation
            import ray
            from panic_tda.embeddings import get_actor_class as get_embedding_actor_class
            
            # Process each embedding model
            for embedding_model in image_embedding_models:
                # Get the actor class and create an actor
                embedding_actor_class = get_embedding_actor_class(embedding_model)
                actor = embedding_actor_class.remote()
                
                try:
                    # Use compute_embeddings to process this batch
                    # Note: compute_embeddings already checks for existing embeddings
                    embedding_ids = ray.get(
                        engine.compute_embeddings.remote(
                            actor, invocation_ids, embedding_model, db_str
                        )
                    )
                    
                    logger.info(
                        f"Processed {len(embedding_ids)} embeddings with {embedding_model} "
                        f"for batch {batch_num}"
                    )
                    
                except Exception as e:
                    logger.error(
                        f"Error creating embeddings with {embedding_model} "
                        f"for batch {batch_num}: {e}"
                    )
            ...
            logger.info(
                f"Progress: {processed}/{total_count} invocations processed "
                f"({batch_num}/{total_batches} batches) - "
                f"Batch time: {batch_time:.1f}s - "
                f"ETA: {eta.strftime('%H:%M:%S')} ({eta_seconds/60:.1f} minutes)"
            )
        
        # Final summary
        total_time = time.time() - start_time
        logger.info(
            f"Completed creating image embeddings for {total_count} IMAGE invocations "
            f"in {total_time/60:.1f} minutes"
        )
        
        # Final check of embeddings created
        for model in image_embedding_models:
            ...
            logger.info(f"{model}: {final_count}/{total_count} embeddings now exist")
    ...
    with get_session_from_connection_string(db_str) as session:
        ...
        for result in results:
            ...
            if verbose:
                ...
                typer.echo(f"  Duration: {naturaldelta(result.duration)}")
                ...
                duration_str = (
                    naturaldelta(result.duration) if result.duration > 0 else "N/A"
                )
                typer.echo(
...
def cluster_status_command(
    ...
    with get_session_from_connection_string(db_str) as session:
        ...
        duration_str = naturaldelta(duration) if duration > 0 else "N/A"
