...
def get_clustering_status(session: Session) -> Dict[UUID, Dict[str, any]]:
    ...
    global_clustering_results = session.exec(
...
def cluster_all_data(
    ...
    if not force:
        ...
        if existing:
            logger.info("Clustering results already exist. Use force=True to re-cluster.")
            return {
    ...
    try:
        ...
        for model in embedding_models:
            model_clusters = clustered_df.filter(
                pl.col("embedding_model") == model
            )["cluster_label"].unique().drop_nulls()
