defmodule PanicTda.LyapunovResult do
  use Ash.Resource,
    domain: PanicTda,
    data_layer: AshSqlite.DataLayer

  sqlite do
    table("lyapunov_results")
    repo(PanicTda.Repo)
  end

  attributes do
    uuid_v7_primary_key(:id)

    attribute :embedding_model, :string do
      allow_nil?(false)
      public?(true)
    end

    attribute :network, {:array, :string} do
      allow_nil?(false)
      public?(true)
    end

    attribute :prompt, :string do
      allow_nil?(false)
      public?(true)
    end

    attribute :lyapunov_data, PanicTda.Types.LyapunovData do
      public?(true)
    end

    attribute :started_at, :utc_datetime_usec do
      allow_nil?(false)
      public?(true)
    end

    attribute :completed_at, :utc_datetime_usec do
      public?(true)
    end

    create_timestamp(:inserted_at)
    update_timestamp(:updated_at)
  end

  relationships do
    belongs_to :experiment, PanicTda.Experiment do
      allow_nil?(false)
      attribute_type(:uuid_v7)
    end
  end

  identities do
    identity :unique_experiment_network_prompt_model,
             [:experiment_id, :network, :prompt, :embedding_model]
  end

  actions do
    defaults([:read, :destroy])

    create :create do
      accept([
        :embedding_model,
        :network,
        :prompt,
        :lyapunov_data,
        :started_at,
        :completed_at,
        :experiment_id
      ])
    end

    update :update do
      accept([:lyapunov_data, :completed_at])
    end
  end

  validations do
    validate {PanicTda.Validations.TimestampOrder, []} do
      on([:create])
    end
  end

  calculations do
    calculate(
      :duration,
      :float,
      expr(fragment("(julianday(?) - julianday(?)) * 86400", completed_at, started_at))
    )
  end
end
