...
| Pooled partition | One microstate partition fitted over every cell's stationary frames and shared by all of them, each cell still getting its own transition matrix. A single cell holds too few frames to support a few hundred microstates on its own, and a shared partition puts every cell on the same ruler, which is what comparing metastable sets across cells requires. `msm_pipeline.py` does this by default; `--partition per-cell` is the comparison, not the analysis. |
